...
- **Description:** Records information about a document uploaded by the user (e.g., Photo ID, Driving License). The actual file upload should happen via `POST /api/upload?destination=documents`.
- **Authentication:** User (JWT Required)
...
- **Description:** Fetches public site settings (e.g., site title, default currency, maintenance mode, global discount).
- **Authentication:** Public
- **Success Response (200 OK):**
  ```json
  {
    ...
    "globalDiscountPercent": "number (0-100, optional)"
  }
  ```
- **Error Responses:**
